...
同じRF操作中に十分な表示領域から狭い表示へ切り替え、その後ふたたび十分な表示領域へ戻った場合は、切り替え前に利用者が移動したPopoverの位置を復元する。狭い表示での折りたたみ状態も同じRF操作中は保持し、十分な表示領域では適用せず、再び狭い表示へ戻った場合に復元する。RFを終了して新しく開始した場合は、狭い表示も展開した状態から開始する。

ソフトウェアキーボードなどによって利用可能な高さが小さくなった場合、展開状態の入力画面とTableを常に同時表示できることは必須としない。入力画面の内容が利用可能な高さへ収まらない場合は入力画面内をスクロールできるようにし、折りたたみ操作へ到達できる状態を維持する。利用者が入力画面を折りたためば、Tableを確認できる状態へ退避できるようにする。
...
RFの反映中は、対象Tableで新しい行DnD、列DnD、またはRFを開始できない。反映完了、取消または失敗によって適切な状態へ戻るまでは、別の並び替え操作へ切り替えない。
