...
`reorder-v1-design.md`で定義する初回案内の表示中は、「行を並び替え」「列を並び替え」とあわせてRFの入口も通常時より視覚的に強調し、利用者が利用できる並び替え手段の入口を確認できるようにする。

初回案内の表示中にRFの入口を選択した場合は、他の並び替え入口を選択した場合と同様に初回案内と入口の強調表示を終了し、その操作環境では表示済みとして扱う。
...
行番号の入力欄には、現在の`tbody`に存在する行の位置範囲を表示する。
...
利用者が行番号の入力を開始した後、その入力内容だけで不正と判断できる場合は、「並び替え」を実行する前でも問題のある入力箇所の近くにエラーを表示する。
...
列は現在存在する列から選択するため、存在しない列番号や解釈できない文字列を利用者が直接入力する方式にはしない。
