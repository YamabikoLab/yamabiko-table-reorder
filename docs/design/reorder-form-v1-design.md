...
並び替えが完了したらRFの入力画面を終了し、完了ダイアログを表示する。

完了ダイアログには、正常に反映されたことを視覚的に確認できる控えめな成功アイコンを添え、次の見出しを表示する。

`反映しました。`

あわせて、何をどこへ移動したかを表示する。

表示例:

- 行: `2行目を5行目の下へ移動しました。`
- 列: `商品名（3列目）を価格（5列目）の左へ移動しました。`

列に利用できるヘッダーがない場合は、列番号で移動内容を表示する。

利用者が完了ダイアログを閉じた後は、並び替え後のTableを通常の編集状態で表示する。

結果確認のためにTableを自動スクロールしたり、移動した行または列を一時的に強調したりしない。
