...
workspace "YTR Reorder v1 Architecture" {
	...
	model {
		...
		RESP_REORDER_MODE = element "Reorder Mode" "Responsibility" "通常編集、行並び替え、列並び替えのモード選択と排他状態を所有する外側の境界。" {
			tags "Responsibility"
		}
		RESP_REORDER_GUIDANCE = element "Reorder Guidance" "Responsibility" "PC / タッチごとの初回案内表示済み状態と、行・列の入口をまとめて提示する共通案内状態を所有する外側の境界。" {
			tags "Responsibility"
		}
		...
		RESP_ROW_PRESENTATION = element "Reorder Presentation" "Responsibility" "移動不可理由と行DnD中の視覚フィードバックを表現する。" {
			tags "Responsibility"
		}
		...
		DEP_001 = RESP_REORDER_MODE -> EXT_WORDPRESS_EDITOR "WordPress Editor上の通常編集と行・列並び替えの排他状態を扱うために必要とする。" {
		...
		DEP_004 = RESP_REORDER_GUIDANCE -> RESP_REORDER_MODE "いずれかの入口選択による案内終了とモード遷移を整合させるために必要とする。" {
			tags "Structural Dependency"
		}
		...
		DEP_016 = RESP_ROW_DND_INTERACTION -> RESP_ROW_REORDER_TARGET_RESOLUTION "Session開始前に移動可能な行を確定するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_017 = RESP_ROW_DND_INTERACTION -> RESP_ROW_DROP_TARGET_RESOLUTION "active DnD中の移動先判定とcomplete時の現在構造への再照合を行うために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_018 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "Sessionが参照するTable同一性と外部Tableの現在利用可否を確認するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_019 = RESP_ROW_DND_INTERACTION -> RESP_ROW_DATA_UPDATE "現在構造でも成立することを確認できた行移動だけをTableへ反映するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_020 = RESP_ROW_DATA_UPDATE -> RESP_ROW_TABLE_INTEGRATION "対応Table Block差を越えて行順を更新するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_021 = RESP_ROW_DATA_UPDATE -> EXT_WORDPRESS_UNDO "成立した1回の行並び替えを1回のUndoで戻せる更新単位を維持するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_022 = RESP_ROW_PRESENTATION -> RESP_EDITOR_DOM_CONTEXT "現在のeditor contextで行DnDの表示を行うために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_023 = RESP_ROW_PRESENTATION -> RESP_ROW_DND_INTERACTION "移動不可理由、active DnD状態、確定、キャンセル、abortを表示状態へ反映するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_024 = RESP_ROW_AUTO_SCROLL -> RESP_ROW_DND_INTERACTION "activeな行DnD状態と終了状態を自動スクロール判断に必要とする。" {
			tags "Structural Dependency"
		}
		DEP_025 = RESP_ROW_AUTO_SCROLL -> RESP_EDITOR_DOM_CONTEXT "現在のeditor contextでスクロール対象を扱うために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_026 = RESP_ROW_AUTO_SCROLL -> EXT_SCROLL_AREA "行DnD中に縦方向へスクロールできる外部領域を必要とする。" {
			tags "Structural Dependency"
		}
		...
		PF_012 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "[recovery] DnD中だけの表示状態を解除する。" {
		...
		PF_015 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DND_INTERACTION "[failure] start処理中に検出されたRow Reorder所有のContractまたはInvariant違反をoperation boundaryへ伝播する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_START_FAILURE_RECOVERY,failure"
		}
		PF_016 = RESP_ROW_REORDER_TARGET_RESOLUTION -> RESP_ROW_DND_INTERACTION "[failure] 移動対象解決中に検出された内部Errorをstart operation boundaryへ伝播する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_START_FAILURE_RECOVERY,failure"
		}
		PF_017 = RESP_ROW_DND_INTERACTION -> RESP_ROW_INPUT_INTERACTION "[recovery] startに属する入力一時状態を破棄し、Sessionを開始せずidleへ戻る。" {
			tags "Process Flow,ProcessFlow_PV_ROW_START_FAILURE_RECOVERY,recovery"
		}
		PF_018 = RESP_ROW_DROP_TARGET_RESOLUTION -> RESP_ROW_DND_INTERACTION "[failure] progressまたはcomplete中の移動先判定で検出された内部Errorをoperation boundaryへ伝播する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_ACTIVE_DND_FAILURE_RECOVERY,failure"
		}
		PF_019 = RESP_ROW_PRESENTATION -> RESP_ROW_DND_INTERACTION "[failure] active DnD中の表示責務で検出された内部Errorをoperation boundaryまたは必要なexecution boundaryから共通中止経路へ渡す。" {
			tags "Process Flow,ProcessFlow_PV_ROW_ACTIVE_DND_FAILURE_RECOVERY,failure"
		}
		PF_020 = RESP_ROW_AUTO_SCROLL -> RESP_ROW_DND_INTERACTION "[failure] active DnD中の自動スクロール責務で検出された内部Errorをoperation boundaryまたは必要なexecution boundaryから共通中止経路へ渡す。" {
			tags "Process Flow,ProcessFlow_PV_ROW_ACTIVE_DND_FAILURE_RECOVERY,failure"
		}
		PF_021 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "[recovery] 自動スクロール一時状態を終了する。" {
		...
		PF_022 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DATA_UPDATE "[failure] 更新処理中に検出された内部ErrorをData Updateへ伝播する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_DATA_UPDATE_FAILURE_RECOVERY,failure"
		}
		PF_023 = RESP_ROW_DATA_UPDATE -> RESP_ROW_DND_INTERACTION "[failure] complete処理を継続できない内部Errorをoperation boundaryへ伝播する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_DATA_UPDATE_FAILURE_RECOVERY,failure"
		}
		...
		RT_023 = RESP_ROW_DND_INTERACTION -> RESP_ROW_DROP_TARGET_RESOLUTION "complete時はSessionの最終移動先を現在構造へ再照合する。" {
		...
		RT_024 = RESP_ROW_DROP_TARGET_RESOLUTION -> RESP_ROW_TABLE_INTEGRATION "現在の対象Table情報を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.2" "現在の対象Table情報を要求する。"
			}
		}
		RT_025 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DROP_TARGET_RESOLUTION "現在のTable情報、または対象Tableが利用できない正常な不在を返す。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.3" "現在のTable情報、または対象Tableが利用できない正常な不在を返す。"
			}
		}
		RT_026 = RESP_ROW_DROP_TARGET_RESOLUTION -> RESP_ROW_DND_INTERACTION "現在構造では移動を確定できない正常な結果を通知する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.4" "現在構造では移動を確定できない正常な結果を通知する。"
			}
		}
		RT_027 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "DnD中だけの表示を解除する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.5" "DnD中だけの表示を解除する。"
			}
		}
		RT_028 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "自動スクロール状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.6" "自動スクロール状態を終了する。"
			}
		}
		RT_029 = RESP_ROW_DND_INTERACTION -> RESP_ROW_INPUT_INTERACTION "入力方式固有のDnD一時状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.7" "入力方式固有のDnD一時状態を終了する。"
			}
		}
		RT_030 = RESP_ROW_AUTO_SCROLL -> RESP_ROW_DND_INTERACTION "通常のoperation boundaryへ伝播できないexecution boundaryで捕捉したErrorを、元のoperation情報とともに同じ共通中止経路へ渡す。" {
		...
		RT_031 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "共通中止経路としてDnD中だけの表示を解除する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
				"runtime.RV_ROW_DND_FAILURE_RECOVERY.step.2" "共通中止経路としてDnD中だけの表示を解除する。"
			}
		}
		RT_032 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "共通中止経路として自動スクロール状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
				"runtime.RV_ROW_DND_FAILURE_RECOVERY.step.3" "共通中止経路として自動スクロール状態を終了する。"
			}
		}
		RT_033 = RESP_ROW_DND_INTERACTION -> RESP_ROW_INPUT_INTERACTION "共通中止経路として入力方式固有のDnD一時状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
	...
	views {
		...
		custom "PV_ROW_EXTERNAL_CHANGE_RECOVERY" {
			...
			include RESP_ROW_INPUT_INTERACTION RESP_ROW_DND_INTERACTION RESP_ROW_TABLE_INTEGRATION RESP_ROW_DROP_TARGET_RESOLUTION RESP_ROW_DATA_UPDATE RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL
		...
		custom "PV_ROW_ACTIVE_DND_FAILURE_RECOVERY" {
			...
			include RESP_ROW_DROP_TARGET_RESOLUTION RESP_ROW_DND_INTERACTION RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL RESP_ROW_INPUT_INTERACTION
		...
		custom "PV_ROW_DATA_UPDATE_FAILURE_RECOVERY" {
			...
			include RESP_ROW_TABLE_INTEGRATION RESP_ROW_DATA_UPDATE RESP_ROW_DND_INTERACTION RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL RESP_ROW_INPUT_INTERACTION
		...
		custom "RV_ROW_DND_COMPLETE" {
			...
			include RESP_ROW_INPUT_INTERACTION RESP_ROW_DND_INTERACTION RESP_ROW_DROP_TARGET_RESOLUTION RESP_ROW_TABLE_INTEGRATION EXT_SUPPORTED_TABLE_BLOCK RESP_ROW_DATA_UPDATE EXT_WORDPRESS_UNDO RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL
			exclude "relationship.tag!=Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.steps" "1=RT_014;2=RT_015;3=RT_009;4=RT_004;5=RT_016;6=RT_017;7=RT_018;8=RT_019;9=RT_020;10=RT_021;11=RT_022"
			}
			autoLayout lr
		}
		...
		custom "RV_ROW_DND_EXTERNAL_ABORT" {
			...
			include RESP_ROW_DND_INTERACTION RESP_ROW_DROP_TARGET_RESOLUTION RESP_ROW_TABLE_INTEGRATION RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL RESP_ROW_INPUT_INTERACTION
			exclude "relationship.tag!=Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.steps" "1=RT_023;2=RT_024;3=RT_025;4=RT_026;5=RT_027;6=RT_028;7=RT_029"
			}
			autoLayout lr
		}
		...
		custom "RV_ROW_DND_FAILURE_RECOVERY" {
			...
			include RESP_ROW_AUTO_SCROLL RESP_ROW_DND_INTERACTION RESP_ROW_PRESENTATION RESP_ROW_INPUT_INTERACTION
			exclude "relationship.tag!=Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
				"runtime.steps" "1=RT_030;2=RT_031;3=RT_032;4=RT_033"
			}
			autoLayout lr
		}
