...
workspace "YTR Reorder v1 Architecture" {
	...
	model {
		...
		RESP_ROW_DND_INTERACTION = element "DnD Interaction" "Responsibility" "行DnDのSession、開始可否判定、移動先判定、operation boundary、確定、中止、回復Lifecycleを所有する。" {
			tags "Responsibility"
		}
		...
		DEP_015 = RESP_ROW_DND_INTERACTION -> RESP_REORDER_MODE "DnD Interactionがモード状態を所有せず、DnD終了後のモードLifecycle判断をReorder Modeの責務として成立させるために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_016 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "start時の行構造取得、complete時の現在構造への再照合、および確定した行移動の反映に必要とする。" {
			tags "Structural Dependency"
		}
		DEP_017 = RESP_ROW_PRESENTATION -> RESP_EDITOR_DOM_CONTEXT "現在のeditor contextで行DnDの表示を行うために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_018 = RESP_ROW_PRESENTATION -> RESP_ROW_DND_INTERACTION "移動不可理由、active DnD状態、終了時の表示解除、およびDesign上の通知要否を表示状態へ反映するために必要とする。" {
			tags "Structural Dependency"
		}
		...
		PF_003 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "start時の行構造取得とcomplete時の現在構造取得・確定済み行移動の反映へ進む。" {
			tags "Process Flow,ProcessFlow_PV_ROW_REORDER_END_TO_END,normal"
		}
		PF_004 = RESP_ROW_TABLE_INTEGRATION -> EXT_SUPPORTED_TABLE_BLOCK "対応Table Blockから行構造を取得し、確定時はtbodyの行順だけを反映する。" {
			tags "Process Flow,ProcessFlow_PV_ROW_REORDER_END_TO_END,normal"
		}
		...
		PF_006 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DND_INTERACTION "[failure] 対象Tableが現在利用できない、または更新開始前に現在更新できないなど、外部Table状態の変化による継続不能・確定不能をoperation boundaryへ合流させる。" {
			tags "Process Flow,ProcessFlow_PV_ROW_EXTERNAL_CHANGE_RECOVERY,failure"
		}
		...
		PF_017 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DND_INTERACTION "[failure] 更新処理中に検出された内部Errorをcomplete operation boundaryへ伝播する。" {
		...
		RT_002 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "現在の対象Table情報を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_START"
			properties {
				"runtime.RV_ROW_DND_START.step.2" "現在の対象Table情報を要求する。"
			}
		}
		RT_003 = RESP_ROW_TABLE_INTEGRATION -> EXT_SUPPORTED_TABLE_BLOCK "現在の対応Table Blockから行構造とTable同一性を取得する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_START,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_START.step.3" "現在の対応Table Blockから行構造とTable同一性を取得する。"
				"runtime.RV_ROW_DND_COMPLETE.step.3" "現在の対応Table Blockから行構造とTable同一性を取得する。"
			}
		}
		RT_004 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "Session開始時は移動対象行のDnD表示を開始し、開始拒否時は必要な理由表示を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_START"
			properties {
				"runtime.RV_ROW_DND_START.step.4" "Session開始時は移動対象行のDnD表示を開始し、開始拒否時は必要な理由表示を要求する。"
			}
		}
		...
		RT_006 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "現在の移動先と周囲行の一時表示を更新する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_PROGRESS"
			properties {
				"runtime.RV_ROW_DND_PROGRESS.step.2" "現在の移動先と周囲行の一時表示を更新する。"
			}
		}
		...
		RT_009 = RESP_ROW_INPUT_INTERACTION -> RESP_ROW_DND_INTERACTION "行DnDのcomplete operationを渡す。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.1" "行DnDのcomplete operationを渡す。"
			}
		}
		RT_010 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "現在のTable同一性と行構造を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.2" "現在のTable同一性と行構造を要求する。"
			}
		}
		RT_011 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "現在も成立することを確認できた場合だけ確定済み行移動の反映を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.4" "現在も成立することを確認できた場合だけ確定済み行移動の反映を要求する。"
			}
		}
		RT_012 = RESP_ROW_TABLE_INTEGRATION -> EXT_SUPPORTED_TABLE_BLOCK "tbodyの行順だけを確定結果として更新する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.5" "tbodyの行順だけを確定結果として更新する。"
			}
		}
		RT_013 = RESP_ROW_TABLE_INTEGRATION -> EXT_WORDPRESS_UNDO "成立した行並び替えを1回のUndoで戻せる更新単位として維持する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.6" "成立した行並び替えを1回のUndoで戻せる更新単位として維持する。"
			}
		}
		RT_014 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "DnD中だけの表示を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.7" "DnD中だけの表示を終了する。"
			}
		}
		RT_015 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "行DnDの自動スクロール状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.8" "行DnDの自動スクロール状態を終了する。"
			}
		}
		RT_016 = RESP_ROW_DND_INTERACTION -> RESP_REORDER_MODE "complete終了後も現在のTableで行並び替えモードを維持できる結果を渡す。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.RV_ROW_DND_COMPLETE.step.9" "complete終了後も現在のTableで行並び替えモードを維持できる結果を渡す。"
			}
		}
		RT_017 = RESP_ROW_DND_INTERACTION -> RESP_ROW_TABLE_INTEGRATION "complete時は現在の対象Table情報を要求する。" {
		...
		RT_018 = RESP_ROW_TABLE_INTEGRATION -> RESP_ROW_DND_INTERACTION "現在のTable情報、または対象Tableが利用できない正常な不在を返す。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.2" "現在のTable情報、または対象Tableが利用できない正常な不在を返す。"
			}
		}
		RT_019 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "DnD中だけの表示を解除し、安全な操作継続不能による終了としてDesignで定義された通知を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.3" "DnD中だけの表示を解除し、安全な操作継続不能による終了としてDesignで定義された通知を要求する。"
			}
		}
		RT_020 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "自動スクロール状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.4" "自動スクロール状態を終了する。"
			}
		}
		RT_021 = RESP_ROW_DND_INTERACTION -> RESP_ROW_INPUT_INTERACTION "入力方式固有のDnD一時状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_EXTERNAL_ABORT"
			properties {
				"runtime.RV_ROW_DND_EXTERNAL_ABORT.step.5" "入力方式固有のDnD一時状態を終了する。"
			}
		}
		RT_022 = RESP_ROW_DND_INTERACTION -> RESP_REORDER_MODE "現在のTableで行並び替えモードを安全に継続できるかという結果を渡す。" {
		...
		RT_023 = RESP_ROW_AUTO_SCROLL -> RESP_ROW_DND_INTERACTION "通常のoperation boundaryへ伝播できないexecution boundaryで捕捉したErrorを、元のoperation情報とともに同じ共通中止経路へ渡す。" {
		...
		RT_024 = RESP_ROW_DND_INTERACTION -> RESP_ROW_PRESENTATION "共通中止経路としてDnD中だけの表示を解除し、Designで定義された異常終了通知を要求する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
				"runtime.RV_ROW_DND_FAILURE_RECOVERY.step.2" "共通中止経路としてDnD中だけの表示を解除し、Designで定義された異常終了通知を要求する。"
			}
		}
		RT_025 = RESP_ROW_DND_INTERACTION -> RESP_ROW_AUTO_SCROLL "共通中止経路として自動スクロール状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
				"runtime.RV_ROW_DND_FAILURE_RECOVERY.step.3" "共通中止経路として自動スクロール状態を終了する。"
			}
		}
		RT_026 = RESP_ROW_DND_INTERACTION -> RESP_ROW_INPUT_INTERACTION "共通中止経路として入力方式固有のDnD一時状態を終了する。" {
			tags "Runtime Interaction,Runtime_RV_ROW_DND_FAILURE_RECOVERY"
			properties {
	...
	views {
		...
		custom "DV_ROW_DND_CORE" {
			...
			include EXT_SUPPORTED_TABLE_BLOCK RESP_REORDER_MODE RESP_ROW_INPUT_INTERACTION RESP_ROW_TABLE_INTEGRATION RESP_ROW_DND_INTERACTION
			exclude "relationship.tag!=Structural Dependency"
			autoLayout lr
		}
		...
		custom "RV_ROW_DND_COMPLETE" {
			...
			include RESP_ROW_INPUT_INTERACTION RESP_ROW_DND_INTERACTION RESP_ROW_TABLE_INTEGRATION EXT_SUPPORTED_TABLE_BLOCK EXT_WORDPRESS_UNDO RESP_ROW_PRESENTATION RESP_ROW_AUTO_SCROLL RESP_REORDER_MODE
			exclude "relationship.tag!=Runtime_RV_ROW_DND_COMPLETE"
			properties {
				"runtime.steps" "1=RT_009;2=RT_010;3=RT_003;4=RT_011;5=RT_012;6=RT_013;7=RT_014;8=RT_015;9=RT_016"
			}
			autoLayout lr
		}
		...
		custom "RV_ROW_DND_EXTERNAL_ABORT" {
			...
			properties {
				"runtime.steps" "1=RT_017;2=RT_018;3=RT_019;4=RT_020;5=RT_021;6=RT_022"
			}
			autoLayout lr
		}
		...
		custom "RV_ROW_DND_FAILURE_RECOVERY" {
			...
			properties {
				"runtime.steps" "1=RT_023;2=RT_024;3=RT_025;4=RT_026;5=RT_022"
			}
			autoLayout lr
		}
