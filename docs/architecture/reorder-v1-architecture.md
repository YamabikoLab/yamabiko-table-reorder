...
| --- | --- |
| Reorder Mode | 通常の Table 編集、行並び替え、列並び替えのどの状態にあるかを管理し、並び替え操作の有効範囲と入口の選択状態を決める。 |
...
Reorder Mode は、通常の Table 編集、行並び替え、列並び替えのいずれが現在有効かを管理する。並び替え入口は Reorder Mode の現在状態に応じて、選択中のモードを利用者が確認できる選択状態を示す。Input Interaction は、現在の並び替え状態のもとで PC とタッチ端末の入力固有の差を扱い、DnD の開始・進行・完了・キャンセルという共通の意味へ変換して DnD Interaction に渡す。
...
1. Reorder Mode が通常、行並び替え、列並び替えの現在状態を保持し、並び替え入口はその状態に応じた選択状態を示す。
...
- 並び替え入口の選択表示は Reorder Mode が所有する現在状態を表現し、独立した並び替えモード状態を所有しない。
...
- 並び替え入口の選択表示は Reorder Mode の現在状態と一致させ、通常状態では両方を非選択、行または列の並び替えモードでは対応する一方だけを選択状態とする。
...
並び替えモードを切り替えた場合は、以後の DnD を切り替え後の方向として扱い、並び替え入口の選択表示も切り替え後の現在状態に追従する。並び替えモードを終了した場合は通常の Table 編集へ戻り、両方の入口を非選択状態とする。
...
通常の Table 編集、行並び替え、列並び替えのいずれが現在有効かを管理する。並び替えの入口から状態を切り替え、現在状態を DnD の有効範囲と利用者が現在のモードを確認するための入口選択表示に提供する。
...
「行を並び替え」「列を並び替え」の選択と並び替えモード終了を受け取り、現在の並び替え状態を並び替え入口の選択表示、Input Interaction、DnD Interaction へ提供する。

並び替え入口の選択表示は、Reorder Mode の現在状態を利用者に示す表現として扱う。通常状態では両方の入口を非選択、行並び替えでは行の入口だけを選択、列並び替えでは列の入口だけを選択する。

通常状態では DnD を有効にせず、行並び替えでは行、列並び替えでは列を DnD 対象として扱える状態を提供する。

#### 依存関係

並び替え入口の選択表示、Input Interaction、DnD Interaction は現在の並び替え状態を Reorder Mode に依存する。First-use Guidance と Reorder Rediscovery は入口が選択されたことを自身の案内終了条件として扱うが、案内状態を Reorder Mode に持たせない。

#### Lifecycle

通常状態から開始する。行または列の入口が選択されると対応する並び替えモードへ移行し、対応する入口だけを選択状態として表現できる状態を提供する。別方向の入口が選択された場合は選択された側へ切り替わり、終了時は通常状態へ戻って両方の入口を非選択状態として表現できる状態を提供する。

#### Invariant

- 同時に有効な並び替えモードは 1 つだけとする。
- 並び替え入口の選択表示は Reorder Mode の現在状態と一致させる。
- 通常状態では両方の入口を非選択状態とする。
- 行並び替えモードでは行の入口だけを選択状態とし、列並び替えモードでは列の入口だけを選択状態とする。
