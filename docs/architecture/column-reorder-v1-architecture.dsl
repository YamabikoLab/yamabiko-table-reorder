...
workspace "YTR Reorder v1 Architecture" {
	...
	model {
		...
		RESP_COLUMN_PRESENTATION = element "Reorder Presentation" "Responsibility" "開始不可、移動対象、垂直挿入位置、周囲列移動、終了通知をColumn Reorderの独立表示として表現する。" {
			tags "Responsibility"
			!script groovy {
				element.setGroup("Column Reorder")
			}
		...
		DEP_010 = RESP_COLUMN_INPUT_INTERACTION -> RESP_COLUMN_TARGET_RESOLUTION "入力開始候補を第一段階の現在制約で解決するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_011 = RESP_COLUMN_INPUT_INTERACTION -> EXT_DND_ENGINE "開始可能な候補だけを物理DnD開始候補として一時登録するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_012 = RESP_COLUMN_INPUT_INTERACTION -> RESP_COLUMN_PRESENTATION "第一段階でDesign上の開始拒否理由が返った場合に一回性の利用者向け通知へ接続するために必要とする。" {
			tags "Structural Dependency"
		}
		DEP_013 = RESP_COLUMN_DND_ENGINE_INTEGRATION -> RESP_COLUMN_INPUT_INTERACTION "DnD Engine境界の配下で列開始入力を有効化し、開始候補登録を接続するために必要とする。" {
			tags "Structural Dependency"
		}
		...
		PF_012 = EXT_DND_ENGINE -> RESP_COLUMN_DND_ENGINE_INTEGRATION "[failure] 物理DnDがcancelまたは継続不能として終了する。" {
			tags "Process Flow,ProcessFlow_PV_COLUMN_EXTERNAL_CHANGE_RECOVERY,failure"
		}
		PF_013 = RESP_COLUMN_DND_ENGINE_INTEGRATION -> RESP_COLUMN_DND_INTERACTION "[recovery] 物理DnDの終了種別を列DnD Sessionのcancelへ接続する。" {
			tags "Process Flow,ProcessFlow_PV_COLUMN_EXTERNAL_CHANGE_RECOVERY,recovery"
		}
		PF_014 = RESP_COLUMN_TABLE_INTEGRATION -> RESP_COLUMN_DND_INTERACTION "[failure] complete時の現在Table利用不能または更新不能を安全な確定不能結果として返す。" {
			tags "Process Flow,ProcessFlow_PV_COLUMN_EXTERNAL_CHANGE_RECOVERY,failure"
		}
		PF_015 = RESP_COLUMN_DND_INTERACTION -> RESP_COLUMN_PRESENTATION "[recovery] DnD中表示を終了し、Designで通知対象となる確定不能だけを一回性通知へ反映する。" {
			tags "Process Flow,ProcessFlow_PV_COLUMN_EXTERNAL_CHANGE_RECOVERY,recovery"
		}
		PF_016 = RESP_COLUMN_DND_INTERACTION -> RESP_REORDER_MODE "[recovery] Session終了後に対象Tableで列並び替えを継続できるかだけを共通モード状態へ反映する。" {
			tags "Process Flow,ProcessFlow_PV_COLUMN_EXTERNAL_CHANGE_RECOVERY,recovery"
		}
	...
	views {
		...
		systemLandscape "DV_COLUMN_EDITOR_INTEGRATION" {
			...
			include EXT_WORDPRESS_EDITOR EXT_WORDPRESS_PREFERENCES RESP_REORDER_MODE RESP_REORDER_GUIDANCE RESP_EDITOR_DOM_CONTEXT RESP_WORDPRESS_REORDER_INTEGRATION RESP_REORDER_GUIDANCE_INTEGRATION RESP_COLUMN_DND_ENGINE_INTEGRATION
			exclude "relationship.tag!=Structural Dependency"
			autoLayout lr
		}
		...
		systemLandscape "DV_COLUMN_DATA_UPDATE" {
			...
			include EXT_SUPPORTED_TABLE_BLOCK EXT_WORDPRESS_UNDO RESP_COLUMN_TABLE_INTEGRATION RESP_COLUMN_DND_INTERACTION
			exclude "relationship.tag!=Structural Dependency"
			autoLayout lr
		}
