...
| 21 | Reorder Constraints | 並び替え制約 | `RowReorderConstraints`, `blockedBoundaries` | 1回のDnD中に移動先判定で利用する構造上の制約。 | `blockedBoundaries`は対象方向の結合セル内部のinsertion boundary indexを重複なし・昇順で保持する。 |
| 22 | Column DnD Layout Availability | 列DnD配置可用性 | `ColumnDndLayoutAvailability`, `resolveColumnDndLayoutAvailability()` | 要求時点のEditor DOMで、観測済み論理列境界がColumn DnDに利用できる横方向の1次元配置を形成するか判定する責務。 | 表示方式、特定論理列の移動可否、移動先解決、状態、監視、UIは所有しない。Toolbar表示用snapshotはWordPress Reorder Integrationが所有し、DnD開始直前は現在DOMから再評価する。 |
| 23 | Chat Reorder | チャット並び替え | `ChatReorder`, `ReorderChat` | 自然言語入力をAIでRF Command Textへ正規化し、既存RFへ接続するPoC入力方式。 | AIはTable更新権限を持たず、RFより下流の成立性判断を所有しない。 |
| 24 | RF Command Text | RFコマンド文字列 | `ChatReorderCommand`, `parseChatReorderCommand()` | Chat ReorderでAIが返す1行の最小Command表現。 | `row`、`column`、`ask`だけを許可し、strict parser通過前は未信頼入力として扱う。 |
| 25 | AI Normalization Ability | AI正規化Ability | `yamabiko-table-reorder/normalize-reorder-command`, `requestChatReorderCommand()` | Chat Reorderの自然言語入力とcompact Table contextをWordPress Abilities API経由でserver-sideへ渡し、RF Command Textへ正規化する実行境界。 | AI実通信はWordPress 7.0のWP AI Client / Connectorsへ委ねる。YTRはprovider固有認証を持たず、Table更新Abilityも公開しない。 |
