...
| 3 | Reorder Mode | 並び替えモード | `reorderModeIntegration`, `rowReorderMode`, `select()`, `observeTable()`, `notifyTableInactive()` | Table編集と行・列の並び替えを排他的に表す現在のモード。 | `edit`、`row`、`column`を持つ状態概念。`Reorder Kind`とは区別する。 |
