...
| 23 | Virtual Insertion Gap | 仮想挿入領域 | `RowVirtualInsertionGapLayout`, `yamabiko-table-reorder-virtual-insertion-gap` | DnD中の有効な移動先境界を基準に、drop後に並び替え対象が占める領域の量感を実Tableのlayout変更なしで示すPresentation。 | Row Reorderでは移動元行の実測高さを使い、下方向移動では境界の上側、上方向移動では境界の下側へ表示する。 |
