...
| 23 | Chat Reorder | チャット並び替え | `ChatReorder`, `ReorderChatPopover` | 自然言語入力をAIでRF Command Textへ正規化し、既存RFへ接続するPoC入力方式。 | AIはTable更新権限を持たず、RFより下流の成立性判断を所有しない。 |
...
| 25 | AI Transport | AI通信境界 | `ChatAiTransport` | Chat Reorderが1回分のAI requestを外部providerへ渡すためのprovider非依存境界。 | API keyやprovider固有認証はYTR本体に保持せず、PoCではWordPress filterから注入する。 |
