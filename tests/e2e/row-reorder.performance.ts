...
// 通常E2Eとは独立したQR-01計測。巨大Table全体の所要時間に合否閾値を設けない。
function frameOwner( frame: { url: string; functionName: string } ) {
	...
	if ( frame.url.includes( '/flexible-table-block/' ) || frame.url.includes( '/block-library/' ) ) {
		return 'Table Block';
	}
...
for ( const name of [ 'core/table', 'flexible-table-block/table' ] as TableName[] ) {
	test( `${ name } 1000 x 20 separates block updates from YTR drag phases`, async ( {
		admin,
		page,
		editor,
		browser,
		...
		const measurements: { phase: string; wallMs: number; cpuSelfMs: Record< string, number > }[] =
			[];
		...
		try {
			...
			const destination = await pointIn( rows.nth( 3 ), 0.8 );
