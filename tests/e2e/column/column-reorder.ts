 ...
 * @param locator    操作対象。
 * @param horizontal 対象内の横位置（左端0、右端1）。
...
export async function pointIn(
	...
	return { x: box.x + box.width * horizontal, y: box.y + box.height * vertical };
}
...
export async function touchInput( page: Page ) {
	...
	return {
		...
		scrollHorizontally: async ( point: Point ) => {
			...
			for ( let step = 1; step <= 8; step++ ) {
				await session.send( 'Input.dispatchTouchEvent', {
					type: 'touchMove',
					touchPoints: [ { x: point.x - step * 50, y: point.y, id: 1 } ],
				} );
			}
			await session.send( 'Input.dispatchTouchEvent', {
				type: 'touchEnd',
