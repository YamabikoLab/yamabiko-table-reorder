...
test( '同一 Architecture Model から同一 DSL を生成する', () => {
	...
	assert.match( first, /relationship "normal" \{[^}]*style solid/u );
	...
	assert.match( first, /relationship "Structural Dependency" \{[^}]*style solid/u );
	assert.match( first, /EXT_EDITOR -> RESP_INPUT "入力する。"/u );
	assert.match( first, /tags "Runtime Interaction,Runtime_RV_DND_START"/u );
	assert.match( first, /relationship "Runtime Interaction" \{[^}]*style solid/u );
...
test( 'Failure / Recovery Process Flow をタイトル・ラベル・線種で区別する', () => {
	...
	assert.match( dsl, /relationship "failure" \{[^}]*color #b42318[^}]*style dashed/u );
	assert.match( dsl, /relationship "recovery" \{[^}]*color #b54708[^}]*style dotted/u );
} );
