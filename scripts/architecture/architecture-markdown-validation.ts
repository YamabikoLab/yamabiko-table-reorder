...
const parseTableHeader = ( tokens: Token[], tableStartIndex: number ): string[] => {
	...
	for ( let index = tableStartIndex + 1; index < tokens.length; index++ ) {
		...
		if ( token.type === 'th_close' ) inCell = false;
	}
	return header;
};

const parseTableColumn = ( tokens: Token[], tableStartIndex: number, columnIndex: number ): string[] => {
	const values: string[] = [];
	let inBody = false;
	...
	for ( let index = tableStartIndex + 1; index < tokens.length; index++ ) {
		...
		if ( token.type === 'tbody_close' || token.type === 'table_close' ) break;
		if ( ! inBody ) continue;
		...
		if ( token.type === 'td_close' ) inCell = false;
	}
	return values;
};

const requireExactHeader = ( actual: string[], expected: readonly string[], item: string ): void => {
	const exact = actual.length === expected.length && actual.every( ( column, index ) => column === expected[ index ] );
	if ( ! exact ) {
		throw new Error( `Architecture validation failed: ${ item } table columns must be exactly: ${ expected.join( ', ' ) }.` );
	}
};

const requirePositiveIntegerRuntimeSteps = ( tokens: Token[], tableStartIndex: number, runtimeScenarioId: string ): void => {
	parseTableColumn( tokens, tableStartIndex, 0 ).forEach( ( step ) => {
		...
		if ( ! positiveIntegerPattern.test( step ) || stepNumber <= 0 ) {
			throw new Error( `Architecture validation failed: Runtime View ${ runtimeScenarioId } Step "${ step }" must be a positive integer.` );
		}
	} );
};
...
export const validateArchitectureMarkdownStructure = ( source: string ): void => {
	...
	for ( let index = 0; index < tokens.length; index++ ) {
		...
		if ( token.type === 'heading_open' ) {
			...
			for ( let deeper = level + 1; deeper <= 6; deeper++ ) headings.delete( deeper );

			if ( level === 2 && [ '3. Context and Scope', '5. Building Block View', '6. Runtime View' ].includes( headingText ) ) {
			...
			if ( level === 4 && headings.get( 2 ) === '4. Solution Strategy' && headings.get( 3 ) === 'Process Flow Views' ) {
				const match = headingText.match( headingIdPattern );
				if ( match === null ) {
					throw new Error( `Architecture validation failed: Process Flow View heading "${ headingText }" requires an embedded process flow ID.` );
				}
			...
			if ( level === 3 && headings.get( 2 ) === '5. Building Block View' ) {
				if ( headingText === 'Dependencies' ) dependenciesHeadingTokenIndex = index;
				if ( headingText === 'Responsibility Details' ) responsibilityDetailsHeading = true;
			}
			if ( level === 3 && headings.get( 2 ) === '6. Runtime View' ) {
				const match = headingText.match( headingIdPattern );
				if ( match === null ) {
					throw new Error( `Architecture validation failed: Runtime View heading "${ headingText }" requires an embedded runtime ID.` );
				}
		...
		if ( level2 === '4. Solution Strategy' && level3 === 'Process Flow Views' && level4 !== undefined ) {
			const match = level4.match( headingIdPattern );
			if ( match !== null ) {
				requireExactHeader( header, expectedHeaders.processFlow, `Process Flow View ${ match[ 2 ] }` );
				processFlowTables.add( match[ 2 ] );
			}
			continue;
		}
		if ( level2 === '5. Building Block View' && level3 === 'Responsibility Inventory' ) {
			requireExactHeader( header, expectedHeaders.responsibilityInventory, 'Responsibility Inventory' );
	...
	if ( ! externalContextTable ) throw new Error( 'Architecture validation failed: External Context table is missing.' );
	if ( ! responsibilityInventoryTable ) throw new Error( 'Architecture validation failed: Responsibility Inventory table is missing.' );
	if ( ! dependenciesTable ) throw new Error( 'Architecture validation failed: Dependencies table is missing.' );
	if ( processFlowViewsHeadingCount > 1 ) throw new Error( 'Architecture validation failed: Process Flow Views heading may appear at most once.' );
	if ( processFlowViewsHeadingCount === 1 && ! processFlowViewsInSolutionStrategy ) {
		throw new Error( 'Architecture validation failed: Process Flow Views must appear under 4. Solution Strategy.' );
	}
	processFlowHeadings.forEach( ( id ) => {
		if ( ! processFlowTables.has( id ) ) {
			throw new Error( `Architecture validation failed: Process Flow View ${ id } table is missing.` );
		}
	} );
	if ( dependencyViewsHeadingCount > 1 ) throw new Error( 'Architecture validation failed: Dependency Views heading may appear at most once.' );
	if ( dependencyViewsHeadingCount === 1 && ! dependencyViewsInBuildingBlockView ) {
		throw new Error( 'Architecture validation failed: Dependency Views must appear immediately after Dependencies.' );
	}
	...
	if ( dependencyViewsHeadingTokenIndex !== null ) {
		if ( dependenciesHeadingTokenIndex === null || dependencyViewsHeadingTokenIndex < dependenciesHeadingTokenIndex ) {
			throw new Error( 'Architecture validation failed: Dependency Views must appear immediately after Dependencies.' );
		}
		...
		if ( interveningHeading ) throw new Error( 'Architecture validation failed: Dependency Views must appear immediately after Dependencies.' );
	}
	if ( ! responsibilityDetailsHeading ) throw new Error( 'Architecture validation failed: Responsibility Details heading is missing.' );
	if ( runtimeScenarioHeadings.size === 0 ) throw new Error( 'Architecture validation failed: Runtime View requires at least one scenario.' );
	runtimeScenarioHeadings.forEach( ( id ) => {
		if ( ! runtimeScenarioTables.has( id ) ) throw new Error( `Architecture validation failed: Runtime View ${ id } table is missing.` );
	} );
};
