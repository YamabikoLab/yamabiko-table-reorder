...
const validateRequiredRows = ( model: ArchitectureModel ): void => {
	...
	if ( model.responsibilities.length === 0 ) {
		throw new Error( 'Architecture validation failed: Responsibility Inventory requires at least one row.' );
	}
	...
	if ( model.responsibilityDetails.length === 0 ) {
		throw new Error( 'Architecture validation failed: Responsibility Details requires at least one responsibility.' );
	}
...
const validateResponsibilityDetails = ( model: ArchitectureModel ): void => {
	const inventory = new Map( model.responsibilities.map( ( responsibility ) => [ responsibility.id, responsibility.name ] ) );
	const seenDetails = new Set< string >();
	model.responsibilityDetails.forEach( ( detail ) => {
		if ( detail.id === null ) {
			throw new Error( `Architecture validation failed: Responsibility Details heading "${ detail.name }" requires a responsibility ID.` );
		}
		...
		if ( inventoryName === undefined ) {
			throw new Error( `Architecture validation failed: Responsibility Details ID "${ detail.id }" is not defined in Responsibility Inventory.` );
		}
		if ( inventoryName !== detail.name ) {
			throw new Error( `Architecture validation failed: Responsibility Details ${ detail.id } name "${ detail.name }" must match Responsibility Inventory name "${ inventoryName}".` );
		}
		if ( seenDetails.has( detail.id ) ) {
			throw new Error( `Architecture validation failed: Responsibility Details contains duplicate ID "${ detail.id }".` );
		}
		seenDetails.add( detail.id );
	} );
	model.responsibilities.forEach( ( responsibility ) => {
		if ( ! seenDetails.has( responsibility.id ) ) {
			throw new Error( `Architecture validation failed: Responsibility ${ responsibility.id } requires a matching Responsibility Details heading.` );
		}
	} );
};
...
const validateDependencies = ( model: ArchitectureModel, elementIds: Set< string > ): void => {
	...
	model.dependencies.forEach( ( dependency, index ) => {
		...
		if ( ! elementIds.has( dependency.dependent ) ) {
			throw new Error( `Architecture validation failed: ${ item } Dependent "${ dependency.dependent }" does not reference an External Context or Responsibility ID.` );
		}
		if ( ! elementIds.has( dependency.dependsOn ) ) {
			throw new Error( `Architecture validation failed: ${ item } Depends on "${ dependency.dependsOn }" does not reference an External Context or Responsibility ID.` );
		}
		const key = `${ dependency.dependent }\u0000${ dependency.dependsOn }`;
		if ( seen.has( key ) ) {
			throw new Error( `Architecture validation failed: duplicate Dependency ${ dependency.dependent } -> ${ dependency.dependsOn }.` );
		}
		seen.add( key );
	} );
};
	...
	model.dependencyViews.forEach( ( view ) => {
		...
		view.includes.forEach( ( id ) => {
			if ( ! elementIds.has( id ) ) {
				throw new Error( `Architecture validation failed: Dependency View ${ view.id } Includes "${ id }" does not reference an External Context or Responsibility ID.` );
			}
		} );
	} );
};
...
const validateProcessFlowViews = ( model: ArchitectureModel, elementIds: Set< string > ): void => {
	model.processFlowViews.forEach( ( view ) => {
		validateStableId( view.id, 'PV_', 'Process Flow View' );
		requireValue( view.name, `Process Flow View ${ view.id } Name` );
		if ( view.edges.length === 0 ) {
			throw new Error( `Architecture validation failed: Process Flow View ${ view.id } requires at least one edge.` );
		}
		const seen = new Set< string >();
		view.edges.forEach( ( edge, index ) => {
			const item = `Process Flow View ${ view.id } row ${ index + 1 }`;
			...
			if ( ! elementIds.has( edge.from ) ) {
				throw new Error( `Architecture validation failed: ${ item} From "${ edge.from }" does not reference an External Context or Responsibility ID.` );
			}
			if ( ! elementIds.has( edge.to ) ) {
				throw new Error( `Architecture validation failed: ${ item } To "${ edge.to }" does not reference an External Context or Responsibility ID.` );
			}
			const key = `${ edge.from }\u0000${ edge.to }`;
			if ( seen.has( key ) ) {
				throw new Error( `Architecture validation failed: Process Flow View ${ view.id } contains duplicate edge ${ edge.from } -> ${ edge.to }.` );
			}
			seen.add( key );
		} );
	} );
...
const validateRuntimeViews = ( model: ArchitectureModel, elementIds: Set< string > ): void => {
	model.runtimeViews.forEach( ( runtimeView ) => {
		...
		if ( runtimeView.steps.length === 0 ) {
			throw new Error( `Architecture validation failed: Runtime View ${ runtimeView.id } requires at least one Step.` );
		}
		runtimeView.steps.forEach( ( step, index ) => {
			const expectedStep = index + 1;
			if ( step.step !== expectedStep ) {
				throw new Error( `Architecture validation failed: Runtime View ${ runtimeView.id } Step must start at 1 and increase without gaps; expected ${ expectedStep } but found ${ step.step }.` );
			}
			...
			if ( ! elementIds.has( step.source ) ) {
				throw new Error( `Architecture validation failed: ${ item } Source "${ step.source }" does not reference an External Context or Responsibility ID.` );
			}
			if ( ! elementIds.has( step.target ) ) {
				throw new Error( `Architecture validation failed: ${ item } Target "${ step.target }" does not reference an External Context or Responsibility ID.` );
			}
		} );
	} );
};

/** Architecture Model の機械可読規則を検証する。 */
