/**
 * Column ReorderテストがProduction Table Integrationへ接続するために、WordPress Block Editor Store境界のTable状態を準備する。
 */
...
/** テストで扱うCore Tableの一行。 */
export type ColumnReorderTestTableRow = {
 ...
 * @public
 */
export const columnReorderTestBlockEditorStore = createReduxStore(
	'yamabiko-table-reorder/test-column-block-editor',
	{
		reducer,
...
register( columnReorderTestBlockEditorStore );
 ...
 */
export const createColumnReorderTestTable = (
	clientId: string,
 ...
 */
export const createColumnReorderTestRow = (
	rowLabel: string,
	columnCount = 5
): ColumnReorderTestTableRow => ( {
 ...
 */
export const setColumnReorderTestTables = ( blocks: Block[] ): void => {
	dispatch( columnReorderTestBlockEditorStore ).setBlocks( blocks );
};
 ...
 * @return 現在Block。存在しない場合はnull。
 */
export const getColumnReorderTestTable = ( clientId: string ): Block | null =>
	select( columnReorderTestBlockEditorStore ).getBlock( clientId );
