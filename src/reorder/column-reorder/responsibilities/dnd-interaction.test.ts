...
describe( 'Column DnD Interaction lifecycle', () => {
	...
	/**
	 * 概要:
	 * - 列移動確定後のReorder Mode継続可否は、complete時の再照合結果を流用せず現在Tableから解決することを確認する。
	 * 事前条件:
	 * - complete時の再照合では移動元・移動先とも成立している。
	 * - 列移動確定後には対象Tableの現在制約を取得できない。
	 * 操作:
	 * - 有効な移動先を保持した状態でcomplete()する。
	 * 期待結果:
	 * - 列移動は確定し、Session終了後の現在TableからColumn Reorder継続不能をReorder Modeへ通知する。
	 */
