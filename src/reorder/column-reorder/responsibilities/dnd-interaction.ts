		...
		( set, get ) => ( {
			...
			complete: ( runtime = null ) => {
				...
				try {
					...
					if ( requiresLargeReorderApply( affectedCellCount, 'column', storage, nowMs ) ) {
						const pendingMove = {
							tableIdentity: session.tableIdentity,
							sourceColumnIndex: session.sourceColumnIndex,
							destinationBoundaryIndex: session.destinationBoundaryIndex,
					...
					const columnMoveApplied = measureDirectReorderApply(
						'column',
						affectedCellCount,
						runtime,
						() => columnTableIntegration.applyColumnMove( move )
					);
...
export const columnDndInteraction: ColumnDndStoreActions = {
	...
	complete: ( runtime = null ) => {
		...
		if ( state.phase !== 'active' ) {
			columnDndStore.getState().complete( runtime );
			return;
		}
