...
jest.mock( '@/reorder/column-reorder/responsibilities/dnd-interaction', () => ( {
	...
	getColumnDndSourceColumnIndex: jest.fn(),
...
const getColumnDndSourceColumnIndexMock = getColumnDndSourceColumnIndex as jest.MockedFunction<
	typeof getColumnDndSourceColumnIndex
>;
...
describe( 'Column DnD React state interface', () => {
	beforeEach( () => {
		...
		getColumnDndSourceColumnIndexMock.mockReturnValue( null );
	 ...
	 *
	 * 事前条件:
	 * - 列DnDはactiveで、移動元論理列は1、現在の有効移動先境界は4である。
	 *
	 * 操作:
	 * - phase、移動元論理列、移動先境界を公開する各Hookをmountする。
	 *
	 * 期待結果:
	 * - 各Hookは対応する現在状態としてactive、1、4を返す。
	 */
	 ...
	 *
	 * 事前条件:
	 * - 各Hookはidle、移動元なし、移動先なしの状態を購読している。
	 *
	 * 操作:
	 * - 共有状態をactive、移動元論理列1、移動先境界4へ変更し、購読者へ状態変更を通知する。
	 *
	 * 期待結果:
	 * - 各HookのReact描画結果がactive、1、4へ更新される。
	 */
	 ...
	 *
	 * 事前条件:
	 * - phase、移動元論理列、移動先境界の各Hookが共有状態を購読している。
	 *
	 * 操作:
	 * - すべてのHookをunmountする。
	 *
