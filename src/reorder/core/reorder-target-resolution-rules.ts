 ...
 */
import type { TableMergedCellStructure } from '@/reorder/foundation/table-integration';

/** 1回のDnD中に移動先判定で利用する構造上の並び替え制約。 */
export type ReorderConstraints = { blockedBoundaries: readonly number[] };
...
export type ReorderTargetAxis = {
	/** @param cell 共通Table構造上の結合セル。 @return 対象方向の0-based開始インデックス。 */
	getStart: ( cell: TableMergedCellStructure ) => number;
	/** @param cell 共通Table構造上の結合セル。 @return 対象方向に占有する行数または列数。 */
