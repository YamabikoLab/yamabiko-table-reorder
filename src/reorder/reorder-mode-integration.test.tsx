...
describe( 'Reorder Mode integration', () => {
	...
	/**
	 * 別Blockへ移動した後、同じTableへ戻る入力でTableを再選択し、
	 * 選択中の並び替えモードを維持できることを確認する。
	 *
	 * 事前条件:
	 * - Table Aで行並び替えモードが選択されている。
	 * - その後、別Blockへ操作対象が移り、Table Aは非選択になっている。
	 *
	 * 操作:
	 * - 非選択のTable Aへ戻る入力を行う。
	 *
	 ...
	 * - Table Aが再選択される。
	 * - 行並び替えモードは終了しない。
	 * - 再表示された行並び替え入口は選択状態を維持する。
	 */
	it( 'when returning from another block to the active reorder table, should reselect the table and keep the reorder mode', () => {
		const TestBlockEdit = ( props: TableBlockEditProps ) => (
			<div data-testid={ `block-${ props.clientId }` }>{ props.name }</div>
		);
		const TestWrapped = withReorderMode( TestBlockEdit );
		...
		/*
		 * 最初にTable Aを選択し、行並び替えモードをONにする。
		 */
		renderTable( root, TestWrapped, tableA );
		...
		/*
		 * 別Blockへ操作対象を移す。
		 * Table Aの並び替えモード自体は維持される。
		 */
		act( () => {
			root.render(
				<>
					<TestWrapped { ...unselectedTableA } />
					<TestWrapped { ...paragraph } />
				</>
			);
		} );
		...
		const tableEdit = container.querySelector< HTMLElement >( '[data-testid="block-table-a"]' );
		const tableWrapper = tableEdit?.parentElement;

		if ( ! tableWrapper ) {
			throw new Error( 'Expected Table A edit wrapper was not rendered.' );
		}

		/*
		 * WordPress Editorが通常の入力からBlockを選択するLifecycleを模擬する。
		 * 入力がpreventDefaultされている場合は、Table Aを再選択できない。
		 */
		let tableReselected = false;

		tableWrapper.parentElement?.addEventListener( 'pointerdown', ( event ) => {
			if ( event.defaultPrevented ) {
				return;
			}

			tableReselected = true;
		} );
		...
		tableWrapper.dispatchEvent( pointerDown );

		expect( tableReselected ).toBe( true );

		/*
		 * Table Aが再選択された結果をReactへ反映する。
		 */
		if ( tableReselected ) {
			renderTable( root, TestWrapped, tableA );
		}
