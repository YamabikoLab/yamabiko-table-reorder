...
jest.mock( 'uuid', () => ( { v4: () => 'reorder-state-transition-test-uuid' } ) );
...
jest.mock( '@wordpress/block-editor', () => {
	...
	const store = createReduxStore( 'test/yamabiko-table-reorder-state-transition-block-editor', {
		reducer: (
...
jest.mock( '@wordpress/preferences', () => {
	const { createReduxStore, register } = jest.requireActual( '@wordpress/data' );
	const store = createReduxStore( 'test/yamabiko-table-reorder-state-transition-preferences', {
		reducer: (
...
describe( 'Reorder Mode WordPress integration state transitions', () => {
	...
	beforeEach( () => {
		...
		updateColumnDndLayoutAvailabilitySnapshot( 'table-b', 'available' );
