/**
 * Flexible Table Block固有の属性構造をTable Integration共通のTable区画表現へ適応する。
 *
 * Flexible Table Blockが保持する`head`、`body`、`foot`と、セルの`rowSpan` / `colSpan`を解釈し、
...
/**
 * 値をFlexible Table Block属性、行、セルとして安全に参照できるオブジェクトか判定する。
 *
 * Flexible Table Block構造を推測しないため、属性を持つデータとして扱えるのは
 * `null`でも配列でもないオブジェクトだけとする。
 *
 * @param value 判定対象の値。
 * @return Flexible Table Blockデータのオブジェクトとして安全に参照できる場合は`true`。
 */
const isRecord = ( value: unknown ): value is Record< string, unknown > =>
	value !== null && typeof value === 'object' && ! Array.isArray( value );

/**
 * Flexible Table Blockセルに保存された結合範囲を、Table Integration共通の占有数へ正規化する。
 *
 * 結合範囲の指定がないセルは通常セルとして1を返す。指定がある場合は1以上の整数だけを受け入れる。
 * Flexible Table Blockデータとして数値文字列を許容し、それ以外は安全に構造を適応できないため`null`とする。
 *
 * @param span Flexible Table Blockセルから取得した結合範囲値。
...
const parseSpan = ( span: unknown ): number | null => {
	...
	// Flexible Table Blockが結合範囲として表現できる数値または数値文字列以外は受け入れない。
...
/**
 * Flexible Table Block固有の1区画をTable Integration共通の行表現へ正規化する。
 *
 * Flexible Table Blockではセルの結合範囲を`rowSpan`と`colSpan`で表す。区画や行、セルの構造を安全に
 * 解釈できない場合は`null`とし、不完全なデータをTable Integration共通処理へ渡さない。
 *
...
const normalizeRows = (
	...
	// 存在するFlexible Table Block区画は行の一覧として解釈できる必要がある。
	...
	for ( const row of section ) {
		// 各行はセル一覧を持つFlexible Table Block行として解釈できる場合だけ共通表現へ取り込む。
		...
		for ( const cell of row.cells ) {
			// 各セルはFlexible Table Blockの結合範囲属性を安全に参照できるセルデータである必要がある。
			if ( ! isRecord( cell ) ) {
				return null;
			}
...
/**
 * Flexible Table Block固有属性をTable Integration共通のTable区画一覧へ正規化する。
 *
...
const normalizeAttributes = ( attributes: unknown ): TableIntegrationSections | null => {
	// Flexible Table Block属性そのものを安全に参照できない場合は、区画構造を推測しない。
...
/**
 * Flexible Table Block固有属性をTable Integrationへ接続するSupported Block Integration。
 *
 * Flexible Table Blockの属性構造と結合セル表現を共通区画へ適応し、安全に解釈できない場合は`null`を返す。
 */
export const flexibleTableBlockIntegration: SupportedBlockIntegration = {
