...
const isRowInsideRowSpan = ( structure: TableStructure, targetIndex: number ): boolean => {
	...
	// 結合範囲の一部だけを移動しないよう、対象行がいずれかのrowspan占有範囲に含まれるか確認する。
...
/**
 * 候補境界がrowspanの内部にあり、結合範囲を上下へ分断するか判定する。
 *
 * @param structure 判定基準となる正規化済みTable Structure。
...
const doesBoundarySplitRowSpan = ( structure: TableStructure, boundary: number ): boolean => {
	...
	// Reorder Destinationによって結合範囲を分断しないよう、すべてのrowspan占有範囲を確認する。
	return body.rows.some( ( row ) =>
		row.placements.some(
			( placement ) =>
				placement.rowSpan > 1 &&
...
export const resolveRowDropTarget = (
	...
	// 対象行または移動先のどちらかがrowspanの一体性を壊す場合、行DnDは確定候補にできない。
	if (
